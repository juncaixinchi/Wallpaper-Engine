...
// set  particles' initial radius

var customint_global=20;
...
window.wallpaperPropertyListener = {
	applyUserProperties: function(properties) {
        ...
        if (properties.schemecolor) {
        	var schemeDebugElement = document.getElementById('SchemeTest');
        	var schemeColor = properties.schemecolor.value.split(' ');
        	schemeDebugElement.textContent = schemeColor;
        	schemeColor = schemeColor.map(function(c) {
        		return Math.ceil(c * 255);
        	});
        	schemeDebugElement.style.backgroundColor = 'rgb(' + schemeColor + ')';
        }

        // Read custom slider
        if (properties.customint) {
        	customint_global= properties.customint.value;
        }

        // Read custom wallpaper
        if (properties.customimage.value) {
        	...
        	imageElement = document.getElementById('container');
        	imageElement.style.background = 'url('+'background.jpg'+')';
        	imageElement.style.backgroundSize = 'cover';
        }
    }
};
...
demo.spawn = function( x, y ) {
	...
	//设置初始参数
	particle.init( x, y, random( 5, customint_global ) );
